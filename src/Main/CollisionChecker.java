...
public class CollisionChecker {
	...
	public void checkTile(Entity entity) {
		...
		case "left":
			...
			if(entityLeftCol >= 0 && entityBottomRow >= 0 && entityTopRow >= 0) {
				tileNum1 = gp.tileM.mapTileNum[entityTopRow][entityLeftCol];
		...
		case "right":
			...
			if(entityRightCol < gp.maxWorldCol && entityBottomRow >= 0 && entityTopRow >= 0) {
	...
	public void checkZombieJump(Entity entity) {
		int entityLeftWorldX = entity.worldX + entity.solidArea.x;
		int entityRightWorldX = entity.worldX + entity.solidArea.x + entity.solidArea.width;
		int entityTopWorldY = entity.worldY + entity.solidArea.y;
		int entityBottomWorldY = entity.worldY + entity.solidArea.y + entity.solidArea.height;
		...
		case "left":
			...
			if(entityLeftCol >= 0 && entityBottomRow >= 0 && entityTopRow >= 0) {
				tileNum2 = gp.tileM.mapTileNum[entityBottomRow][entityLeftCol];
				if(tileNum2 != 0 && gp.tileM.tile[tileNum2].collision) {
					entity.jumpCollision = true;
				}
			}else
				entity.jumpCollision = false;
			break;
